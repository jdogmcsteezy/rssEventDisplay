...
class AcademicCalanderDisplay(pygame.Surface):
    ...
    def Update(self):
        ...
        if self.scrollX1 < self.scrollBar_Rect.right:
            self.scrollBar.blit(self.scrollBar_content, (self.scrollX1, self.scrollBar_Rect.centery - self.scrollBar_contentRect.centery))
            print('X1', end='')
        if self.scrollX2 < self.scrollBar_Rect.right:
            self.scrollBar.blit(self.scrollBar_content, (self.scrollX2, self.scrollBar_Rect.centery - self.scrollBar_contentRect.centery))
            print('X2', end='')
        print('')
        self.blit(self.scrollBar, (0, self.height * self.titleBar_heightRatio))
        return self
...
def main():
    ...
    ticker = AcademicCalanderDisplay(600, 200)
    testScreen.blit(ticker, (0, 200))
    pygame.display.update()
    run = True
    while(run):
        clock.tick(26)
        testScreen.blit(ticker.Update(), (0, 200))
        pygame.display.update()
